...
while True:
   ...
   if specDB == "":
      specDB = "99"
      error.append(specDBList[specDB]) 
   phase1 = {"1":""}
   for i in range(1 ,len(sheetSpecVIP["B"])+1):
      if sheetSpecVIP["B{}".format(i)].value == None:
         ...
         if sheetSpecVIP["N{}".format(i)].value != "" or sheetSpecVIP["O{}".format(i)].value != "":
            input("中福VIP出現,請通知YT")
         break
      ...
      if url_Number == sheetDownload["A{}".format(i)].value.strip():
         ...
         while True:
            if os.path.isfile(appName +".apk") or os.path.isfile(appName +".apk.crdownload") :
               ...
               error.append("{}下載失敗".format(appName))
               break        
            else:
               sleep(1)
               countTime = countTime + 1 
   ...
   while deZip:
      if os.path.isfile(appName + ".apk"):#反組譯
         subprocess.Popen("{}\\{}\\apktool d -f {}\\{}.apk -o {}\\{}".format(ogCwd ,"apktool" ,os.getcwd() ,appName ,ogCwd ,url_Number) ,stdout=PIPE \
                          ,stderr=PIPE ,stdin=PIPE ,shell = True).stdout.read().decode('utf-8')
      elif os.path.isfile(appName + ".apk.crdownload"):
   ...
   requestUrl = {"requestUrl":"" ,"requestUrl2":"" ,"requestUrl3":"" ,"requestUrl4":""  ,"requestUrl5":""  ,"requestUrl6":""}
   try:
      ...
      for child_of_root in root.findall("string"):
         ...
         elif child['name'] == "requestUrl5":
            requestUrl["requestUrl5"] = child_of_root.text
         elif child['name'] == "requestUrl6":
            requestUrl["requestUrl6"] = child_of_root.text
   except:
      error.append("解析XML錯誤")

   requestUrl1 ,requestUrl2 ,requestUrl3 ,requestUrl4 ,requestUrl5 ,requestUrl6 = "NG" ,"NG" ,"NG" ,"NG" ,"NG" ,"NG"
   for i in phase1:
      if requestUrl["requestUrl5"] != "" and requestUrl["requestUrl6"] != "":
         if phase1[i] in requestUrl["requestUrl"]:
            ...
            requestUrl4 = "OK"
         elif phase1[i] in requestUrl["requestUrl5"]:
            requestUrl5 = "OK"
         elif phase1[i] in requestUrl["requestUrl6"]:
            requestUrl6 = "OK"
